package Perl::APIReference::V5_020_001;
use strict;
use warnings;
...
Usually used via its frontend C<sv_setpvf_mg>.
	...
	void	sv_vsetpvf_mg(SV *const sv,
		              ...
		              va_list *const args)','name' => 'sv_vsetpvf_mg'},'toUPPER_uni' => {'text' => 'Converts the Unicode code point C<cp> to its uppercase version, and
...
(but note, as explained just above, that there may be more.)
	...
	UV	toUPPER_uni(UV cp, U8* s, STRLEN* lenp)','name' => 'toUPPER_uni'},'SvPVutf8' => {'text' => 'Like C<SvPV>, but converts sv to utf8 first if necessary.
...
Unlike with L</vcroak>, C<pat> is not permitted to be null.
	...
	NV	scan_oct(const char* start, STRLEN len,
		         STRLEN* retlen)','name' => 'scan_oct'},'av_arylen_p' => {'text' => '','name' => 'av_arylen_p'},'my_strlcat' => {'text' => 'The C library C<strlcat> if available, or a Perl implementation of it.
...
room for the C<NUL> should be included in C<size>.
	...
	Size_t	my_strlcat(char *dst, const char *src,
		           Size_t size)','name' => 'my_strlcat'},'gv_add_by_type' => {'text' => '','name' => 'gv_add_by_type'},'newGVOP' => {'text' => 'Constructs, checks, and returns an op of any type that involves an
...
rather than in the scope of the debugger itself.)
	...
	CV*	find_runcv(U32 *db_seqp)','name' => 'find_runcv'},'utf8n_to_uvuni' => {'text' => 'Instead use L</utf8_to_uvchr_buf>, or rarely, L</utf8n_to_uvchr>.
...
or C<L<NATIVE_TO_UNI(utf8n_to_uvchr(...))|/utf8n_to_uvchr>>.
	...
	UV	utf8n_to_uvuni(const U8 *s, STRLEN curlen,
		               STRLEN *retlen, U32 flags)','name' => 'utf8n_to_uvuni'},'newSVpvn_utf8' => {'text' => 'Creates a new SV and copies a string (which may contain C<NUL> (C<\\0>)
...
of an SV instead of a string/length pair.
	...
	GV*	gv_fetchmeth_sv_autoload(HV* stash, SV* namesv,
		                         I32 level, U32 flags)','name' => 'gv_fetchmeth_sv_autoload'},'pregcomp' => {'text' => '','name' => 'pregcomp'},'my_cxt_index' => {'text' => '','name' => 'my_cxt_index'},'lex_discard_to' => {'text' => 'NOTE: this function is experimental and may change or be
...
C<mPUSHp> and C<PUSHp>.
	...
	void	mXPUSHp(char* str, STRLEN len)','name' => 'mXPUSHp'},'lex_stuff_sv' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
time you\'ll want to use C<sv_setsv> or one of its many macro front-ends.
	...
	void	sv_replace(SV *const sv, SV *const nsv)','name' => 'sv_replace'},'utf8n_to_uvchr' => {'text' => 'THIS FUNCTION SHOULD BE USED IN ONLY VERY SPECIALIZED CIRCUMSTANCES.
...
NOTE: the perl_ form of this function is deprecated.
	...
	char*	SvPV_nolen(SV* sv)','name' => 'SvPV_nolen'},'alloccopstash' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
more than once.  Does not handle \'set\' magic on the destination SV.
	...
	void	SvSetSV(SV* dsv, SV* ssv)','name' => 'SvSetSV'},'rninstr' => {'text' => '','name' => 'rninstr'},'pad_findmy_pvn' => {'text' => 'Given the name of a lexical variable, find its position in the
...
wrapper for C<strncmp>).
	...
	bool	strnNE(char* s1, char* s2, STRLEN len)','name' => 'strnNE'},'cophh_store_pv' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
(A slightly shorter form is C<av_tindex>.)
	...
	SSize_t	av_top_index(AV *av)','name' => 'av_top_index'},'XPUSHi' => {'text' => 'Push an integer onto the stack, extending the stack if necessary.  Handles
...
"Superseded" by sv_nosharing().
	...
	void	sv_nolocking(SV *sv)','name' => 'sv_nolocking'},'parse_listexpr' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
C<s2>.  Returns true or false.
	...
	bool	strLT(char* s1, char* s2)','name' => 'strLT'},'save_pushptr' => {'text' => '','name' => 'save_pushptr'},'save_clearsv' => {'text' => '','name' => 'save_clearsv'},'av_shift' => {'text' => 'Removes one SV from the start of the array, reducing its size by one and
...
"Superseded" by sv_nosharing().
	...
	void	sv_nounlocking(SV *sv)','name' => 'sv_nounlocking'},'perl_clone_using' => {'text' => '','name' => 'perl_clone_using'},'PerlIO_context_layers' => {'text' => '','name' => 'PerlIO_context_layers'},'lex_unstuff' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
memory which is shared between threads.
	...
	char*	savesharedsvpv(SV *sv)','name' => 'savesharedsvpv'},'CopyD' => {'text' => 'Like C<Copy> but returns dest.  Useful
...
Note that the perl-level function is vaguely deprecated.
	...
	void	sv_reset(const char* s, HV *const stash)','name' => 'sv_reset'},'cophh_delete_pv' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
by the name defined by the I<namegv> parameter.
	...
	OP *	ck_entersub_args_proto_or_list(OP *entersubop,
		                               GV *namegv,
		                               SV *protosv)','name' => 'ck_entersub_args_proto_or_list'},'hv_fetch_ent' => {'text' => 'Returns the hash entry which corresponds to the specified key in the hash.
...
information on how to use this function on tied hashes.
	...
	HE*	hv_fetch_ent(HV *hv, SV *keysv, I32 lval,
		             U32 hash)','name' => 'hv_fetch_ent'},'op_null' => {'text' => 'Neutralizes an op when it is no longer needed, but is still linked to from
other ops.

	void	op_null(OP *o)','name' => 'op_null'},'gv_fetchpvn_flags' => {'text' => '','name' => 'gv_fetchpvn_flags'},'ENTER' => {'text' => 'Opening bracket on a callback.  See C<LEAVE> and L<perlcall>.
...
Memory obtained by this should B<ONLY> be freed with L<"Safefree">.
	...
	void	Newxz(void* ptr, int nitems, type)','name' => 'Newxz'},'SVt_PVHV' => {'text' => 'Type flag for hashes.  See L</svtype>.','name' => 'SVt_PVHV'},'reentrant_init' => {'text' => '','name' => 'reentrant_init'},'save_pushptrptr' => {'text' => '','name' => 'save_pushptrptr'},'save_gp' => {'text' => '','name' => 'save_gp'},'parse_block' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
be zero.
	...
	OP *	parse_block(U32 flags)','name' => 'parse_block'},'toLOWER_uni' => {'text' => 'Converts the Unicode code point C<cp> to its lowercase version, and
stores that in UTF-8 in C<s>, and its length in bytes in C<lenp>.  Note
that the buffer pointed to by C<s> needs to be at least C<UTF8_MAXBYTES_CASE+1>
...
(but note, as explained just above, that there may be more.)
	...
	UV	toLOWER_uni(UV cp, U8* s, STRLEN* lenp)','name' => 'toLOWER_uni'},'SvRX' => {'text' => 'Convenience macro to get the REGEXP from a SV.  This is approximately
...
set as required.
	...
	OP *	newASSIGNOP(I32 flags, OP *left, I32 optype,
		            OP *right)','name' => 'newASSIGNOP'},'toFOLD' => {'text' => 'Converts the specified character to foldcase.  If the input is anything but an
...
Latin1 range, as the full generality of L</toFOLD_uni> is needed there.)
	...
	U8	toFOLD(U8 ch)','name' => 'toFOLD'},'SvSetMagicSV_nosteal' => {'text' => 'Like C<SvSetSV_nosteal>, but does any set magic required afterwards.
...
by the name defined by the I<namegv> parameter.
	...
	int	getcwd_sv(SV* sv)','name' => 'getcwd_sv'},'newSVpvs_share' => {'text' => 'Like C<newSVpvn_share>, but takes a literal C<NUL>-terminated string instead of
a string/length pair and omits the hash parameter.

	SV*	newSVpvs_share(const char* s)','name' => 'newSVpvs_share'},'newSVpv' => {'text' => 'Creates a new SV and copies a string (which may contain C<NUL> (C<\\0>)
...
then.
	...
	char*	fbm_instr(unsigned char* big,
		          ...
		          U32 flags)','name' => 'fbm_instr'},'toFOLD_uni' => {'text' => 'Converts the Unicode code point C<cp> to its foldcase version, and
stores that in UTF-8 in C<s>, and its length in bytes in C<lenp>.  Note
that the buffer pointed to by C<s> needs to be at least C<UTF8_MAXBYTES_CASE+1>
...
(but note, as explained just above, that there may be more.)
	...
	UV	toFOLD_uni(UV cp, U8* s, STRLEN* lenp)','name' => 'toFOLD_uni'},'XPUSHn' => {'text' => 'Push a double onto the stack, extending the stack if necessary.  Handles
\'set\' magic.  Uses C<TARG>, so C<dTARGET> or C<dXSTARG> should be called to
declare it.  Do not call multiple C<TARG>-oriented macros to return lists
...
When you replace this variable, it is considered a good practice to store the possibly previously installed hook and that you recall it inside your own.
	...
	Perl_ophook_t	PL_opfreehook','name' => 'PL_opfreehook'},'PadlistARRAY' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
numbers >= 1, as the 0th entry is not guaranteed to remain usable.
	...
	PAD **	PadlistARRAY(PADLIST padlist)','name' => 'PadlistARRAY'},'dump_all' => {'text' => 'Dumps the entire optree of the current program starting at C<PL_main_root> to 
C<STDERR>.  Also dumps the optrees for all visible subroutines in
C<PL_defstash>.

	void	dump_all()','name' => 'dump_all'},'ninstr' => {'text' => '','name' => 'ninstr'},'sv_vcatpvf' => {'text' => 'Processes its arguments like C<vsprintf> and appends the formatted output
...
See also C<PL_sv_yes> and C<PL_sv_no>.
	...
	SV *	boolSV(bool b)','name' => 'boolSV'},'PERL_SYS_INIT3' => {'text' => 'Provides system-specific tune up of the C runtime environment necessary to
run Perl interpreters.  This should be called only once, before creating
any Perl interpreters.

	void	PERL_SYS_INIT3(int *argc, char*** argv,
		               char*** env)','name' => 'PERL_SYS_INIT3'},'upg_version' => {'text' => 'In-place upgrade of the supplied SV to a version object.
...
char * and length parameters.  C<flags> is currently unused.
	...
	void	gv_init_sv(GV* gv, HV* stash, SV* namesv,
		           U32 flags)','name' => 'gv_init_sv'},'my_bcopy' => {'text' => '','name' => 'my_bcopy'},'PL_parser-E<gt>bufptr' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
L</lex_read_unichar>.','name' => 'PL_parser-E<gt>bufptr'},'G_NOARGS' => {'text' => 'Indicates that no arguments are being sent to a callback.  See
L<perlcall>.','name' => 'G_NOARGS'},'amagic_deref_call' => {'text' => '','name' => 'amagic_deref_call'},'lex_start' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
DISALLOW flags.
	...
	U8*	uvchr_to_utf8_flags(U8 *d, UV uv, UV flags)','name' => 'uvchr_to_utf8_flags'},'save_hash' => {'text' => '','name' => 'save_hash'},'sv_isa' => {'text' => 'Returns a boolean indicating whether the SV is blessed into the specified
class.  This does not check for subtypes; use C<sv_derived_from> to verify
an inheritance relationship.

	int	sv_isa(SV* sv, const char *const name)','name' => 'sv_isa'},'cophh_free' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
valid UTF-8; if the original SV was bytes, the pattern should be too.
	...
	void	sv_catpvf(SV *const sv, const char *const pat,
		          ...)','name' => 'sv_catpvf'},'cop_fetch_label' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
The flags pointer may be set to C<SVf_UTF8> or 0.
	...
	void	XSRETURN_PV(char* str)','name' => 'XSRETURN_PV'},'utf8_to_uvuni' => {'text' => 'DEPRECATED!  It is planned to remove this function from a
...
See L</utf8n_to_uvchr> for details on when the REPLACEMENT CHARACTER is returned.
	...
	UV	utf8_to_uvuni(const U8 *s, STRLEN *retlen)','name' => 'utf8_to_uvuni'},'pad_alloc' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
the same as C<av_clear(av)>.
	...
	void	av_fill(AV *av, SSize_t fill)','name' => 'av_fill'},'croak_sv' => {'text' => 'This is an XS interface to Perl\'s C<die> function.
...
more convenient.
	...
	void	croak_sv(SV *baseex)','name' => 'croak_sv'},'warn_nocontext' => {'text' => '','name' => 'warn_nocontext'},'SvIOK_only' => {'text' => 'Tells an SV that it is an integer and disables all other OK bits.

	void	SvIOK_only(SV* sv)','name' => 'SvIOK_only'},'MoveD' => {'text' => 'Like C<Move> but returns dest.  Useful
for encouraging compilers to tail-call
optimise.
...
number may use \'_\' characters to separate digits.
	...
	UV	grok_bin(const char* start, STRLEN* len_p,
		         I32* flags, NV *result)','name' => 'grok_bin'},'parse_barestmt' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
be zero.
	...
	MAGIC*	mg_find(const SV* sv, int type)','name' => 'mg_find'},'safesysfree' => {'text' => '','name' => 'safesysfree'},'PL_parser-E<gt>linestr' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
through normal scalar means.','name' => 'PL_parser-E<gt>linestr'},'BhkENTRY_set' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
Handles \'get\' magic, but not \'set\' magic.  See C<sv_catpvn_mg>.
	...
	void	sv_catpvn(SV *dsv, const char *sstr, STRLEN len)','name' => 'sv_catpvn'},'my_socketpair' => {'text' => '','name' => 'my_socketpair'},'blockhook_register' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
or will otherwise be the global one.
	...
	SV *	find_rundefsv()','name' => 'find_rundefsv'},'sv_utf8_downgrade' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
see C<hv_iterinit>.
	...
	SV*	hv_iterkeysv(HE* entry)','name' => 'hv_iterkeysv'},'sv_catpv_flags' => {'text' => 'Concatenates the C<NUL>-terminated string onto the end of the string which is
in the SV.
...
See also C<PUSHp>, C<mXPUSHp> and C<XPUSHp>.
	...
	void	mPUSHp(char* str, STRLEN len)','name' => 'mPUSHp'},'lex_stuff_pv' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
C<G_DISCARD> version.
	...
	SV*	av_delete(AV *av, SSize_t key, I32 flags)','name' => 'av_delete'},'utf8_distance' => {'text' => 'Returns the number of UTF-8 characters between the UTF-8 pointers C<a>
and C<b>.
...
this function and become part of the constructed op tree.
	...
	void	sv_setpvs(SV* sv, const char* s)','name' => 'sv_setpvs'},'uvoffuni_to_utf8_flags' => {'text' => 'THIS FUNCTION SHOULD BE USED IN ONLY VERY SPECIALIZED CIRCUMSTANCES.
...
For details, see the description for L</uvchr_to_utf8_flags>>.
	...
	U8*	uvoffuni_to_utf8_flags(U8 *d, UV uv, UV flags)','name' => 'uvoffuni_to_utf8_flags'},'cxinc' => {'text' => '','name' => 'cxinc'},'SvUVx' => {'text' => 'Coerces the given SV to an unsigned integer and
...
instead.
	...
	void	sv_clear(SV *const orig_sv)','name' => 'sv_clear'},'lex_read_space' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
more convenient.
	...
	void	sv_setpv_mg(SV *const sv, const char *const ptr)','name' => 'sv_setpv_mg'},'whichsig' => {'text' => '','name' => 'whichsig'},'lex_next_chunk' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
contain SV_GMAGIC, then it does an mg_get() first.
	...
	bool	sv_2bool_flags(SV *sv, I32 flags)','name' => 'sv_2bool_flags'},'sync_locale' => {'text' => 'Changing the program\'s locale should be avoided by XS code.  Nevertheless,
...
to do so, before returning to Perl code.
	...
	void	sync_locale()','name' => 'sync_locale'},'malloc' => {'text' => '','name' => 'malloc'},'av_undef' => {'text' => 'Undefines the array.  Frees the memory used by the av to store its list of
scalars.  If any destructors are triggered as a result, the av itself may
be freed.

	void	av_undef(AV *av)','name' => 'av_undef'},'parse_label' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
Instead, use the function L</wrap_op_checker>.','name' => 'PL_check'},'pack_cat' => {'text' => 'The engine implementing pack() Perl function.  Note: parameters
...
of.  Hence, it croaks on read-only values.
	...
	void	sv_force_normal_flags(SV *const sv,
		                      const U32 flags)','name' => 'sv_force_normal_flags'},'cophh_fetch_pv' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
a string/length pair.
	...
	SV *	cophh_fetch_pv(const COPHH *cophh,
		               const char *key, U32 hash,
		               U32 flags)','name' => 'cophh_fetch_pv'},'tmps_grow' => {'text' => '','name' => 'tmps_grow'},'is_utf8_char' => {'text' => 'DEPRECATED!  It is planned to remove this function from a
future release of Perl.  Do not use it for new code; remove it from
existing code.
...
this function.
	...
	SV *	vmess(const char *pat, va_list *args)','name' => 'vmess'},'my_stat' => {'text' => '','name' => 'my_stat'},'my_setenv' => {'text' => '','name' => 'my_setenv'},'newSV_type' => {'text' => 'Creates a new SV, of the type specified.  The reference count for the new SV
is set to 1.
...
and faster.
	...
	void	SvREFCNT_inc_void_NN(SV* sv)','name' => 'SvREFCNT_inc_void_NN'},'Perl_signbit' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
See L<perlcall/LIGHTWEIGHT CALLBACKS>.
		...
		PUSH_MULTICALL;','name' => 'PUSH_MULTICALL'},'save_svref' => {'text' => '','name' => 'save_svref'},'newNULLLIST' => {'text' => 'Constructs, checks, and returns a new C<stub> op, which represents an
empty list expression.

	OP *	newNULLLIST()','name' => 'newNULLLIST'},'sv_pos_b2u_flags' => {'text' => 'Converts the offset from a count of bytes from the start of the string, to
...
C<SV_GMAGIC|SV_CONST_RETURN> to handle magic.
	...
	STRLEN	sv_pos_b2u_flags(SV *const sv,
		                 STRLEN const offset, U32 flags)','name' => 'sv_pos_b2u_flags'},'sv_pos_u2b_flags' => {'text' => 'Converts the offset from a count of UTF-8 chars from
the start of the string, to a count of the equivalent number of bytes; if
lenp is non-zero, it does the same to lenp, but this time starting from
...
operation.
	...
	SV*	hv_iternextsv(HV *hv, char **key, I32 *retlen)','name' => 'hv_iternextsv'},'lex_read_to' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
for this element.  Does not use C<TARG>.  See also C<PUSHs> and C<mXPUSHs>.
	...
	void	mPUSHs(SV* sv)','name' => 'mPUSHs'},'dump_packsubs' => {'text' => 'Dumps the optrees for all visible subroutines in C<stash>.

	void	dump_packsubs(const HV* stash)','name' => 'dump_packsubs'},'GvSV' => {'text' => 'Return the SV from the GV.
...
C<isPRINT_LC_uvchr>, and C<isPRINT_LC_utf8>.
	...
	bool	isPRINT(char ch)','name' => 'isPRINT'},'vform' => {'text' => '','name' => 'vform'},'get_op_descs' => {'text' => '','name' => 'get_op_descs'},'safesyscalloc' => {'text' => '','name' => 'safesyscalloc'},'PL_curpad' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.


Points directly to the body of the L</PL_comppad> array.
(I.e., this is C<PAD_ARRAY(PL_comppad)>.)','name' => 'PL_curpad'},'POPi' => {'text' => 'Pops an integer off the stack.

	IV	POPi','name' => 'POPi'},'parse_stmtseq' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
Usually accessed via the C<SvPVutf8_nolen> macro.
	...
	char*	sv_2pvutf8_nolen(SV* sv)','name' => 'sv_2pvutf8_nolen'},'gv_HVadd' => {'text' => '','name' => 'gv_HVadd'},'custom_op_xop' => {'text' => 'Return the XOP structure for a given custom op.  This macro should be
...
more information on how to use this function on tied arrays.
	...
	SV**	av_store(AV *av, SSize_t key, SV *val)','name' => 'av_store'},'reg_named_buff_scalar' => {'text' => '','name' => 'reg_named_buff_scalar'},'unlnk' => {'text' => '','name' => 'unlnk'},'toUPPER_utf8' => {'text' => 'Converts the UTF-8 encoded character at C<p> to its uppercase version, and
stores that in UTF-8 in C<s>, and its length in bytes in C<lenp>.  Note
that the buffer pointed to by C<s> needs to be at least C<UTF8_MAXBYTES_CASE+1>
bytes since the uppercase version may be longer than the original character.
...
The input character at C<p> is assumed to be well-formed.
	...
	UV	toUPPER_utf8(U8* p, U8* s, STRLEN* lenp)','name' => 'toUPPER_utf8'},'av_fetch' => {'text' => 'Returns the SV at the specified index in the array.  The C<key> is the
...
The rough perl equivalent is C<$myarray[$idx]>.
	...
	SV**	av_fetch(AV *av, SSize_t key, I32 lval)','name' => 'av_fetch'},'SvNOK_only' => {'text' => 'Tells an SV that it is a double and disables all other OK bits.
...
case-insensitively in the current locale; false otherwise.
	...
	I32	foldEQ_locale(const char* a, const char* b,
		              I32 len)','name' => 'foldEQ_locale'},'cophh_2hv' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
and faster.
	...
	void	SvREFCNT_inc_simple_void_NN(SV* sv)','name' => 'SvREFCNT_inc_simple_void_NN'},'rsignal_state' => {'text' => '','name' => 'rsignal_state'},'av_len' => {'text' => 'Same as L</av_top_index>.  Note that, unlike what the name implies, it returns
...
expect.
	...
	SSize_t	av_len(AV *av)','name' => 'av_len'},'sv_unmagic' => {'text' => 'Removes all magic of type C<type> from an SV.
...
need the wrapper function - usually this is a direct call to C<sprintf>.
	...
	int	my_sprintf(char *buffer, const char *pat, ...)','name' => 'my_sprintf'},'to_utf8_fold' => {'text' => 'Instead use L</toFOLD_utf8>.
...
computed.
	...
	bool	hv_exists_ent(HV *hv, SV *keysv, U32 hash)','name' => 'hv_exists_ent'},'PL_comppad' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
values for the pad for the currently-executing code.','name' => 'PL_comppad'},'atfork_unlock' => {'text' => '','name' => 'atfork_unlock'},'SvNIOK' => {'text' => 'Returns a U32 value indicating whether the SV contains a number, integer or
double.
...
via, C<mro::method_changed_in(classname)>.
	...
	void	mro_method_changed_in(HV* stash)','name' => 'mro_method_changed_in'},'toTITLE_uni' => {'text' => 'Converts the Unicode code point C<cp> to its titlecase version, and
stores that in UTF-8 in C<s>, and its length in bytes in C<lenp>.  Note
that the buffer pointed to by C<s> needs to be at least C<UTF8_MAXBYTES_CASE+1>
...
(but note, as explained just above, that there may be more.)
	...
	UV	toTITLE_uni(UV cp, U8* s, STRLEN* lenp)','name' => 'toTITLE_uni'},'unpack_str' => {'text' => 'The engine implementing unpack() Perl function.  Note: parameters strbeg,
...
The pointer to the PV of the C<dsv> is returned.
	...
	int	sv_unmagicext(SV *const sv, const int type,
		              MGVTBL *vtbl)','name' => 'sv_unmagicext'},'newSVpvs' => {'text' => 'Like C<newSVpvn>, but takes a literal C<NUL>-terminated string instead of a
string/length pair.

	SV*	newSVpvs(const char* s)','name' => 'newSVpvs'},'SvREFCNT_dec' => {'text' => 'Decrements the reference count of the given SV.  I<sv> may be NULL.

	void	SvREFCNT_dec(SV* sv)','name' => 'SvREFCNT_dec'},'realloc' => {'text' => '','name' => 'realloc'},'toLOWER_utf8' => {'text' => 'Converts the UTF-8 encoded character at C<p> to its lowercase version, and
stores that in UTF-8 in C<s>, and its length in bytes in C<lenp>.  Note
that the buffer pointed to by C<s> needs to be at least C<UTF8_MAXBYTES_CASE+1>
bytes since the lowercase version may be longer than the original character.
...
The input character at C<p> is assumed to be well-formed.
	...
	UV	toLOWER_utf8(U8* p, U8* s, STRLEN* lenp)','name' => 'toLOWER_utf8'},'POPs' => {'text' => 'Pops an SV off the stack.
...
C<gv_fetchmeth> to C<call_sv> applies equally to these functions.
	...
	GV*	gv_fetchmethod_autoload(HV* stash,
		                        const char* name,
		                        I32 autoload)','name' => 'gv_fetchmethod_autoload'},'get_mstats' => {'text' => '','name' => 'get_mstats'},'op_dump' => {'text' => 'Dumps the optree starting at OP C<o> to C<STDERR>.

	void	op_dump(const OP *o)','name' => 'op_dump'},'savestack_grow' => {'text' => '','name' => 'savestack_grow'},'debstackptrs' => {'text' => '','name' => 'debstackptrs'},'sv_eq' => {'text' => 'Returns a boolean indicating whether the strings in the two SVs are
...
children can still follow the full lexical scope chain.
	...
	void	StructCopy(type *src, type *dest, type)','name' => 'StructCopy'},'sv_catpvf_nocontext' => {'text' => '','name' => 'sv_catpvf_nocontext'},'save_padsv_and_mortalize' => {'text' => '','name' => 'save_padsv_and_mortalize'},'gv_autoload_sv' => {'text' => '','name' => 'gv_autoload_sv'},'cophh_delete_pvs' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
NOTE: the perl_ form of this function is deprecated.
	...
	CV*	get_cvn_flags(const char* name, STRLEN len,
		              I32 flags)','name' => 'get_cvn_flags'},'XopENTRY_set' => {'text' => 'Set a member of the XOP structure.  I<which> is a cpp token
...
C<PUSHs> and C<PUSHmortal>.
	...
	void	XPUSHs(SV* sv)','name' => 'XPUSHs'},'set_numeric_radix' => {'text' => '','name' => 'set_numeric_radix'},'newIO' => {'text' => '','name' => 'newIO'},'toFOLD_utf8' => {'text' => 'Converts the UTF-8 encoded character at C<p> to its foldcase version, and
stores that in UTF-8 in C<s>, and its length in bytes in C<lenp>.  Note
that the buffer pointed to by C<s> needs to be at least C<UTF8_MAXBYTES_CASE+1>
bytes since the foldcase version may be longer than the original character.
...
The input character at C<p> is assumed to be well-formed.
	...
	UV	toFOLD_utf8(U8* p, U8* s, STRLEN* lenp)','name' => 'toFOLD_utf8'},'PadnameUTF8' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
Unlike with L</croak>, C<pat> is not permitted to be null.
	...
	void	warn(const char *pat, ...)','name' => 'warn'},'PerlIO_get_bufsiz' => {'text' => '','name' => 'PerlIO_get_bufsiz'},'fp_dup' => {'text' => '','name' => 'fp_dup'},'hek_dup' => {'text' => '','name' => 'hek_dup'},'pmop_dump' => {'text' => '','name' => 'pmop_dump'},'newANONATTRSUB' => {'text' => '','name' => 'newANONATTRSUB'},'CvPADLIST' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
For state vars, SVs_PADSTALE is overloaded to mean \'not yet initialised\'.
	...
	PADLIST * CvPADLIST(CV *cv)','name' => 'CvPADLIST'},'SVt_PVGV' => {'text' => 'Type flag for typeglobs.  See L</svtype>.','name' => 'SVt_PVGV'},'HvNAMEUTF8' => {'text' => 'Returns true if the name is in UTF8 encoding.

	unsigned char HvNAMEUTF8(HV *stash)','name' => 'HvNAMEUTF8'},'sv_catpvf_mg_nocontext' => {'text' => '','name' => 'sv_catpvf_mg_nocontext'},'reg_named_buff_all' => {'text' => '','name' => 'reg_named_buff_all'},'PerlIO_close' => {'text' => '','name' => 'PerlIO_close'},'EXTEND' => {'text' => 'Used to extend the argument stack for an XSUB\'s return values.  Once
used, guarantees that there is room for at least C<nitems> to be pushed
onto the stack.

	void	EXTEND(SP, SSize_t nitems)','name' => 'EXTEND'},'newOP' => {'text' => 'Constructs, checks, and returns an op of any base type (any type that
...
private flags).
	...
	bool	SvTRUE(SV* sv)','name' => 'SvTRUE'},'regdump' => {'text' => '','name' => 'regdump'},'my_pclose' => {'text' => '','name' => 'my_pclose'},'hv_iternext_flags' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
See also L</is_utf8_string_loc>() and L</is_utf8_string>().
	...
	bool	is_utf8_string_loclen(const U8 *s, STRLEN len,
		                      const U8 **ep, STRLEN *el)','name' => 'is_utf8_string_loclen'},'BhkENABLE' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
pointer.
	...
	void	BhkENABLE(BHK *hk, which)','name' => 'BhkENABLE'},'call_method' => {'text' => 'Performs a callback to the specified Perl method.  The blessed object must
...
Perl equivalent: C<unshift @myarray, ( (undef) x $n );>
	...
	void	av_unshift(AV *av, SSize_t num)','name' => 'av_unshift'},'save_set_svflags' => {'text' => '','name' => 'save_set_svflags'},'SvREFCNT_inc_NN' => {'text' => 'Same as SvREFCNT_inc, but can only be used if you know I<sv>
...
added to it.  A pointer to the new hash is returned.
	...
	HV *	hv_copy_hints_hv(HV *ohv)','name' => 'hv_copy_hints_hv'},'do_pmop_dump' => {'text' => '','name' => 'do_pmop_dump'},'sv_utf8_decode' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
Usually accessed via the C<SvPVbyte_nolen> macro.
	...
	char*	sv_2pvbyte_nolen(SV* sv)','name' => 'sv_2pvbyte_nolen'},'PadlistMAX' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
flexible routine.
	...
	int	av_tindex(AV* av)','name' => 'av_tindex'},'cophh_store_pvs' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
string/length pair, and no precomputed hash.
	...
	COPHH *	cophh_store_pvs(const COPHH *cophh,
		                ...
		                U32 flags)','name' => 'cophh_store_pvs'},'sys_intern_clear' => {'text' => '','name' => 'sys_intern_clear'},'doref' => {'text' => '','name' => 'doref'},'scan_bin' => {'text' => 'For backwards compatibility.  Use C<grok_bin> instead.

	NV	scan_bin(const char* start, STRLEN len,
		         STRLEN* retlen)','name' => 'scan_bin'},'Nullcv' => {'text' => 'Null CV pointer.

(deprecated - use C<(CV *)NULL> instead)','name' => 'Nullcv'},'PadlistREFCNT' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
string/length pair.
	...
	void	sv_catpvs_nomg(SV* sv, const char* s)','name' => 'sv_catpvs_nomg'},'lex_bufutf8' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
C<isPSXSPC_LC_uvchr>, and C<isPSXSPC_LC_utf8>.
	...
	SV*	SvRV(SV* sv)','name' => 'SvRV'},'hv_scalar' => {'text' => 'Evaluates the hash in scalar context and returns the result.  Handles magic
...
number may use \'_\' characters to separate digits.
	...
	UV	grok_oct(const char* start, STRLEN* len_p,
		         I32* flags, NV *result)','name' => 'grok_oct'},'MARK' => {'text' => 'Stack marker variable for the XSUB.  See C<dMARK>.','name' => 'MARK'},'newSVpvn' => {'text' => 'Creates a new SV and copies a string into it, which may contain C<NUL> characters
...
extended.
	...
	void	av_extend(AV *av, SSize_t key)','name' => 'av_extend'},'sv_peek' => {'text' => '','name' => 'sv_peek'},'SVt_REGEXP' => {'text' => 'Type flag for regular expressions.  See L</svtype>.','name' => 'SVt_REGEXP'},'hv_iternext' => {'text' => 'Returns entries from a hash iterator.  See C<hv_iterinit>.
...
See also L</is_ascii_string>(), L</is_utf8_string_loclen>(), and L</is_utf8_string_loc>().
	...
	bool	is_utf8_string(const U8 *s, STRLEN len)','name' => 'is_utf8_string'},'PadnamelistARRAY' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
could potentially warn under some level of strict-ness.
	...
	bool	sv_destroyable(SV *sv)','name' => 'sv_destroyable'},'save_aelem_flags' => {'text' => '','name' => 'save_aelem_flags'},'PadlistNAMES' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
C<xsubpp>.  It is the same as using the more explicit XS_EXTERNAL macro.','name' => 'XS'},'gv_check' => {'text' => '','name' => 'gv_check'},'sv_setpvn' => {'text' => 'Copies a string (possibly containing embedded C<NUL> characters) into an SV.
...
again) that hopefully catches attempts to access uninitialized memory.
	...
	void	PoisonWith(void* dest, int nitems, type,
		           U8 byte)','name' => 'PoisonWith'},'av_create_and_unshift_one' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
being zero.  See C<SvROK_off>.
	...
	void	sv_unref(SV* sv)','name' => 'sv_unref'},'moreswitches' => {'text' => '','name' => 'moreswitches'},'cophh_fetch_pvs' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
and smaller.
	...
	SV*	SvREFCNT_inc_simple_NN(SV* sv)','name' => 'SvREFCNT_inc_simple_NN'},'toTITLE_utf8' => {'text' => 'Converts the UTF-8 encoded character at C<p> to its titlecase version, and
stores that in UTF-8 in C<s>, and its length in bytes in C<lenp>.  Note
that the buffer pointed to by C<s> needs to be at least C<UTF8_MAXBYTES_CASE+1>
bytes since the titlecase version may be longer than the original character.
...
The input character at C<p> is assumed to be well-formed.
	...
	UV	toTITLE_utf8(U8* p, U8* s, STRLEN* lenp)','name' => 'toTITLE_utf8'},'newHV' => {'text' => 'Creates a new HV.  The reference count is set to 1.

	HV*	newHV()','name' => 'newHV'},'SVt_PVFM' => {'text' => 'Type flag for formats.  See L</svtype>.','name' => 'SVt_PVFM'},'gv_dump' => {'text' => '','name' => 'gv_dump'},'my_strlcpy' => {'text' => 'The C library C<strlcpy> if available, or a Perl implementation of it.
This operates on C C<NUL>-terminated strings.
...
to C<dst>, C<NUL>-terminating the result if C<size> is not 0.
	...
	Size_t	my_strlcpy(char *dst, const char *src,
		           Size_t size)','name' => 'my_strlcpy'},'strNE' => {'text' => 'Test two strings to see if they are different.  Returns true or
false.

	bool	strNE(char* s1, char* s2)','name' => 'strNE'},'PerlIO_fill' => {'text' => '','name' => 'PerlIO_fill'},'my_failure_exit' => {'text' => '','name' => 'my_failure_exit'},'caller_cx' => {'text' => 'The XSUB-writer\'s equivalent of L<caller()|perlfunc/caller>.  The
...
frame for the sub call itself.
	...
	const PERL_CONTEXT * caller_cx(
	                     ...
	                     )','name' => 'caller_cx'},'cophh_copy' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
Perl equivalent: C<my @array;>.
	...
	AV*	newAV()','name' => 'newAV'},'bytes_from_utf8' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
in utf8 (i.e., US-ASCII on non-EBCDIC machines).
	...
	U8*	bytes_from_utf8(const U8 *s, STRLEN *len,
		                bool *is_utf8)','name' => 'bytes_from_utf8'},'croak_memory_wrap' => {'text' => '','name' => 'croak_memory_wrap'},'SvPV_nomg_nolen' => {'text' => 'Like C<SvPV_nolen> but doesn\'t process magic.
...
Perl equivalent: C<my @new_array = ($scalar1, $scalar2, $scalar3...);>
	...
	AV*	av_make(SSize_t size, SV **strp)','name' => 'av_make'},'SvUV' => {'text' => 'Coerces the given SV to an unsigned integer and returns it.  See C<SvUVx>
for a version which guarantees to evaluate sv only once.

	UV	SvUV(SV* sv)','name' => 'SvUV'},'hv_name_set' => {'text' => '','name' => 'hv_name_set'},'cophh_store_pvn' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
negative the key is assumed to be in UTF-8-encoded Unicode.
	...
	bool	hv_exists(HV *hv, const char *key, I32 klen)','name' => 'hv_exists'},'my_popen' => {'text' => '','name' => 'my_popen'},'mg_length' => {'text' => 'DEPRECATED!  It is planned to remove this function from a
future release of Perl.  Do not use it for new code; remove it from
existing code.
...
up past C<e>, croaks.
	...
	void	SvIOK_only_UV(SV* sv)','name' => 'SvIOK_only_UV'},'clone_params_new' => {'text' => '','name' => 'clone_params_new'},'sv_vcatpvfn' => {'text' => 'void	sv_vcatpvfn(SV *const sv, const char *const pat,
		            const STRLEN patlen,
		            va_list *const args,
		            SV **const svargs, const I32 svmax,
		            bool *const maybe_tainted)','name' => 'sv_vcatpvfn'},'BhkDISABLE' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
C<isUPPER_LC_uvchr>, and C<isUPPER_LC_utf8>.
	...
	void	XopENABLE(XOP *xop, which)','name' => 'XopENABLE'},'lex_peek_unichar' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
B<private> setting.  Use C<SvNOK> instead.
	...
	U32	SvNOKp(SV* sv)','name' => 'SvNOKp'},'PadnamePV' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
no threads.
	...
	int	nothreadhook()','name' => 'nothreadhook'},'utf8_to_uvchr' => {'text' => 'DEPRECATED!  It is planned to remove this function from a
future release of Perl.  Do not use it for new code; remove it from
existing code.
...
See L</utf8n_to_uvchr> for details on when the REPLACEMENT CHARACTER is returned.
	...
	UV	utf8_to_uvchr(const U8 *s, STRLEN *retlen)','name' => 'utf8_to_uvchr'},'bytes_to_utf8' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
scalar.
	...
	bool	SvIsCOW_shared_hash(SV* sv)','name' => 'SvIsCOW_shared_hash'},'start_subparse' => {'text' => '','name' => 'start_subparse'},'mg_size' => {'text' => '','name' => 'mg_size'},'PadnamelistMAX' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
The index of the last pad name.
	...
	void	sv_setuv_mg(SV *const sv, const UV u)','name' => 'sv_setuv_mg'},'sys_intern_init' => {'text' => '','name' => 'sys_intern_init'},'SVt_INVLIST' => {'text' => 'Type flag for scalars.  See L</svtype>.','name' => 'SVt_INVLIST'},'XS_VERSION' => {'text' => 'The version identifier for an XS module.  This is usually
handled automatically by C<ExtUtils::MakeMaker>.  See C<XS_VERSION_BOOTCHECK>.','name' => 'XS_VERSION'},'find_rundefsvoffset' => {'text' => 'DEPRECATED!  It is planned to remove this function from a
future release of Perl.  Do not use it for new code; remove it from
existing code.
...
NOTE: the perl_ form of this function is deprecated.
	...
	PADOFFSET find_rundefsvoffset()','name' => 'find_rundefsvoffset'},'sv_setpv' => {'text' => 'Copies a string into an SV.  The string must be terminated with a C<NUL>
character.
Does not handle \'set\' magic.  See C<sv_setpv_mg>.

	void	sv_setpv(SV *const sv, const char *const ptr)','name' => 'sv_setpv'},'hv_placeholders_set' => {'text' => '','name' => 'hv_placeholders_set'},'parse_fullstmt' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
C<isIDFIRST_LC>, C<isIDFIRST_LC_uvchr>, and C<isIDFIRST_LC_utf8>.
	...
	bool	isIDFIRST(char ch)','name' => 'isIDFIRST'},'sv_catpv' => {'text' => 'Concatenates the C<NUL>-terminated string onto the end of the string which is
in the SV.
...
value, or 0 to ask for it to be computed.
	...
	SV*	hv_delete_ent(HV *hv, SV *keysv, I32 flags,
		              U32 hash)','name' => 'hv_delete_ent'},'PL_keyword_plugin' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
the Perl core) will normally return C<KEYWORD_PLUGIN_DECLINE>.','name' => 'PL_keyword_plugin'},'LINKLIST' => {'text' => 'Given the root of an optree, link the tree in execution order using the
...
least an C<UNOP>.
	...
	OP*	LINKLIST(OP *o)','name' => 'LINKLIST'},'utf8_to_uvuni_buf' => {'text' => 'DEPRECATED!  It is planned to remove this function from a
future release of Perl.  Do not use it for new code; remove it from
existing code.
...
class name for a C++ XS constructor.  This is always a C<char*>.  See C<THIS>.
	...
	char*	CLASS','name' => 'CLASS'},'newAVREF' => {'text' => '','name' => 'newAVREF'},'foldEQ_latin1' => {'text' => '','name' => 'foldEQ_latin1'},'HeKLEN' => {'text' => 'If this is negative, and amounts to C<HEf_SVKEY>, it indicates the entry
...
C<SvPV_force> and C<SvPV_force_nomg>
	...
	char*	sv_pvn_force_flags(SV *const sv,
		                   STRLEN *const lp,
		                   const I32 flags)','name' => 'sv_pvn_force_flags'},'sv_setpvf_mg_nocontext' => {'text' => '','name' => 'sv_setpvf_mg_nocontext'},'parse_termexpr' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
C<mXPUSHn>.
	...
	void	PUSHn(NV nv)','name' => 'PUSHn'},'to_utf8_title' => {'text' => 'Instead use L</toTITLE_utf8>.

	UV	to_utf8_title(const U8 *p, U8* ustrp,
		              STRLEN *lenp)','name' => 'to_utf8_title'},'PerlIO_unread' => {'text' => '','name' => 'PerlIO_unread'},'PadMAX' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
the SV.  Does not use C<TARG>.  See also C<XPUSHs> and C<mPUSHs>.
	...
	long	POPl','name' => 'POPl'},'scan_version' => {'text' => 'Returns a pointer to the next character after the parsed
...
it doesn\'t.
	...
	const char* scan_version(const char *s, SV *rv, bool qv)','name' => 'scan_version'},'toLOWER_LC' => {'text' => 'Converts the specified character to lowercase using the current locale\'s rules,
if possible; otherwise returns the input character itself.

	U8	toLOWER_LC(U8 ch)','name' => 'toLOWER_LC'},'hv_common_key_len' => {'text' => '','name' => 'hv_common_key_len'},'vnumify' => {'text' => 'Accepts a version object and returns the normalized floating
...
statement boundaries.  See also C<sv_newmortal> and C<sv_2mortal>.
	...
	SV*	sv_mortalcopy(SV *const oldsv)','name' => 'sv_mortalcopy'},'cophh_fetch_sv' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
string/length pair.
	...
	SV *	cophh_fetch_sv(const COPHH *cophh, SV *key,
		               U32 hash, U32 flags)','name' => 'cophh_fetch_sv'},'PadARRAY' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
The C array of pad entries.
	...
	void	sv_catpvn_nomg(SV* sv, const char* ptr,
		               STRLEN len)','name' => 'sv_catpvn_nomg'},'newHVhv' => {'text' => '','name' => 'newHVhv'},'newSVpvs_flags' => {'text' => 'Like C<newSVpvn_flags>, but takes a literal C<NUL>-terminated string instead of
a string/length pair.
...
of an SV instead of a string/length pair.
	...
	PADOFFSET pad_findmy_sv(SV *name, U32 flags)','name' => 'pad_findmy_sv'},'cophh_delete_sv' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
Perl equivalent: C<exists($myarray[$key])>.
	...
	bool	av_exists(AV *av, SSize_t key)','name' => 'av_exists'},'SvCUR' => {'text' => 'Returns the length of the string which is in the SV.  See C<SvLEN>.
...
instead of a string/length pair.
	...
	bool	sv_derived_from_pv(SV* sv,
		                   ...
		                   U32 flags)','name' => 'sv_derived_from_pv'},'savepvs' => {'text' => 'Like C<savepvn>, but takes a literal C<NUL>-terminated string instead of a
string/length pair.
	...
	void	sv_catpv_nomg(SV* sv, const char* ptr)','name' => 'sv_catpv_nomg'},'ZeroD' => {'text' => 'Like C<Zero> but returns dest.  Useful
for encouraging compilers to tail-call
optimise.
...
C<SPAGAIN>.','name' => 'SP'},'sv_catpvn_mg' => {'text' => 'Like C<sv_catpvn>, but also handles \'set\' magic.
	...
	void	sv_catpvn_mg(SV *sv, const char *ptr,
		             STRLEN len)','name' => 'sv_catpvn_mg'},'op_lvalue' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
such as C<$$x = 5> which might have to vivify a reference in C<$x>.
	...
	HV*	gv_stashpvs(const char* name, I32 create)','name' => 'gv_stashpvs'},'vcmp' => {'text' => 'Version object aware cmp.  Both operands must already have been 
...
destination, C<nitems> is the number of items, and C<type> is the type.
	...
	void	Zero(void* dest, int nitems, type)','name' => 'Zero'},'SvSTASH' => {'text' => 'Returns the stash of the SV.
...
cope with complex macro expressions.  Always use the macro instead.
	...
	IV	sv_iv(SV* sv)','name' => 'sv_iv'},'PL_parser-E<gt>bufend' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
the buffer\'s contents.','name' => 'PL_parser-E<gt>bufend'},'PerlIO_error' => {'text' => '','name' => 'PerlIO_error'},'pv_escape' => {'text' => 'Escapes at most the first "count" chars of pv and puts the results into
...
It calls sv_2bool_flags with the SV_GMAGIC flag.
	...
	bool	sv_2bool(SV *const sv)','name' => 'sv_2bool'},'lex_grow_linestr' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
part of the buffer.
	...
	U32	SvOOK(SV* sv)','name' => 'SvOOK'},'SvPV_set' => {'text' => 'Set the value of the PV pointer in C<sv> to the C<NUL>-terminated string
...
NOTE: the perl_ form of this function is deprecated.
	...
	void	PoisonNew(void* dest, int nitems, type)','name' => 'PoisonNew'},'op_scope' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
seen before (i.e., suppress "Used once" warnings).
	...
	void	gv_init_pvn(GV* gv, HV* stash, const char* name,
		            STRLEN len, U32 flags)','name' => 'gv_init_pvn'},'newATTRSUB' => {'text' => '','name' => 'newATTRSUB'},'gv_fetchmeth_sv' => {'text' => 'Exactly like L</gv_fetchmeth_pvn>, but takes the name string in the form
of an SV instead of a string/length pair.

	GV*	gv_fetchmeth_sv(HV* stash, SV* namesv,
		                I32 level, U32 flags)','name' => 'gv_fetchmeth_sv'},'Copy' => {'text' => 'The XSUB-writer\'s interface to the C C<memcpy> function.  The C<src> is the
source, C<dest> is the destination, C<nitems> is the number of items, and
C<type> is the type.  May fail on overlapping copies.  See also C<Move>.

	void	Copy(void* src, void* dest, int nitems, type)','name' => 'Copy'},'newSVpv_share' => {'text' => 'Like C<newSVpvn_share>, but takes a C<NUL>-terminated string instead of a
string/length pair.
	...
	bool	SvIOK_notUV(SV* sv)','name' => 'SvIOK_notUV'},'deb' => {'text' => '','name' => 'deb'},'PL_parser-E<gt>linestart' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
a newline; the function L</lex_read_to> handles this detail.','name' => 'PL_parser-E<gt>linestart'},'is_safe_syscall' => {'text' => 'Test that the given C<pv> doesn\'t contain any internal C<NUL> characters.
...
Used by the IS_SAFE_SYSCALL() macro.
	...
	bool	is_safe_syscall(const char *pv, STRLEN len,
		                const char *what,
		                const char *op_name)','name' => 'is_safe_syscall'},'pad_add_name_pvn' => {'text' => 'Allocates a place in the currently-compiling pad for a named lexical
...
flags can be OR\'ed together:
	...
	PADOFFSET pad_add_name_pvn(const char *namepv,
	                           ...
	                           HV *typestash, HV *ourstash)','name' => 'pad_add_name_pvn'},'he_dup' => {'text' => '','name' => 'he_dup'},'gv_fullname3' => {'text' => '','name' => 'gv_fullname3'},'OP_TYPE_IS_OR_WAS' => {'text' => 'Returns true if the given OP is not a NULL pointer and
...
the NULL pointer check.
	...
	bool	OP_TYPE_IS_OR_WAS(OP *o, Optype type)','name' => 'OP_TYPE_IS_OR_WAS'},'SVt_PVNV' => {'text' => 'Type flag for scalars.  See L</svtype>.','name' => 'SVt_PVNV'},'mPUSHu' => {'text' => 'Push an unsigned integer onto the stack.  The stack must have room for this
...
obtained from the GV with the C<GvCV> macro.
	...
	GV*	gv_fetchmeth_pvn(HV* stash, const char* name,
		                 STRLEN len, I32 level,
		                 U32 flags)','name' => 'gv_fetchmeth_pvn'},'cophh_new_empty' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
C<mXPUSHp> instead.  See also C<PUSHp> and C<mPUSHp>.
	...
	void	XPUSHp(char* str, STRLEN len)','name' => 'XPUSHp'},'av_create_and_push' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
Do not use frivolously.
	...
	void	sv_setpvn_mg(SV *const sv,
		             ...
		             const STRLEN len)','name' => 'sv_setpvn_mg'},'toLOWER' => {'text' => 'Converts the specified character to lowercase.  If the input is anything but an
ASCII uppercase character, that input character itself is returned.  Variant
C<toLOWER_A> is equivalent.

	U8	toLOWER(U8 ch)','name' => 'toLOWER'},'PadlistNAMESMAX' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
want to upgrade the SV.
	...
	SV*	new_version(SV *ver)','name' => 'new_version'},'PerlIO_seek' => {'text' => '','name' => 'PerlIO_seek'},'free_tmps' => {'text' => '','name' => 'free_tmps'},'parse_arithexpr' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
Returns a pointer to the prettified text as held by dsv.
	...
	void	sv_setnv_mg(SV *const sv, const NV num)','name' => 'sv_setnv_mg'},'utf8_to_bytes' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
string/length pair.
	...
	SV *	sv_setref_pvs(const char* s)','name' => 'sv_setref_pvs'},'re_dup_guts' => {'text' => '','name' => 're_dup_guts'},'si_dup' => {'text' => '','name' => 'si_dup'},'uvuni_to_utf8_flags' => {'text' => 'Instead you almost certainly want to use L</uvchr_to_utf8> or
L</uvchr_to_utf8_flags>>.
...
takes ownership of one reference to it.
	...
	OP *	newSVOP(I32 type, I32 flags, SV *sv)','name' => 'newSVOP'},'cophh_fetch_pvn' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
Use macro PAD_SV instead of calling this function directly.
	...
	SV *	pad_sv(PADOFFSET po)','name' => 'pad_sv'},'amagic_call' => {'text' => '','name' => 'amagic_call'},'op_free' => {'text' => 'Free an op.  Only use this when an op is no longer linked to from any
optree.

	void	op_free(OP *o)','name' => 'op_free'},'PadnameLEN' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
longer than 2Gb.
	...
	void	sv_pos_b2u(SV *const sv, I32 *const offsetp)','name' => 'sv_pos_b2u'},'sv_utf8_upgrade_flags_grow' => {'text' => 'Like sv_utf8_upgrade_flags, but has an additional parameter C<extra>, which is
...
Returns the number of bytes in the converted string (not including the spares).
	...
	STRLEN	sv_utf8_upgrade_flags_grow(SV *const sv,
		                           const I32 flags,
		                           STRLEN extra)','name' => 'sv_utf8_upgrade_flags_grow'},'SVt_IV' => {'text' => 'Type flag for scalars.  See L</svtype>.','name' => 'SVt_IV'},'SvIsCOW' => {'text' => 'Returns a U32 value indicating whether the SV is Copy-On-Write (either shared
hash key scalars, or full Copy On Write scalars if 5.9.0 is configured for
COW).

	U32	SvIsCOW(SV* sv)','name' => 'SvIsCOW'},'reentrant_size' => {'text' => '','name' => 'reentrant_size'},'printf_nocontext' => {'text' => '','name' => 'printf_nocontext'},'save_freesv' => {'text' => '','name' => 'save_freesv'},'sv_catsv' => {'text' => 'Concatenates the string from SV C<ssv> onto the end of the string in SV
C<dsv>.  If C<ssv> is null, does nothing; otherwise modifies only C<dsv>.
...
of an SV instead of a string/length pair.
	...
	PADOFFSET pad_add_name_sv(SV *name, U32 flags,
	                          HV *typestash, HV *ourstash)','name' => 'pad_add_name_sv'},'to_utf8_upper' => {'text' => 'Instead use L</toUPPER_utf8>.

	UV	to_utf8_upper(const U8 *p, U8* ustrp,
		              STRLEN *lenp)','name' => 'to_utf8_upper'},'newSVpadname' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
if necessary.  If the flags include SV_GMAGIC, it handles get-magic, too.
	...
	I32	sv_eq_flags(SV* sv1, SV* sv2, const U32 flags)','name' => 'sv_eq_flags'},'PUSHi' => {'text' => 'Push an integer onto the stack.  The stack must have room for this element.
Handles \'set\' magic.  Uses C<TARG>, so C<dTARGET> or C<dXSTARG> should be
called to declare it.  Do not call multiple C<TARG>-oriented macros to 
...
C<mXPUSHi>.
	...
	void	SvLEN_set(SV* sv, STRLEN len)','name' => 'SvLEN_set'},'to_utf8_lower' => {'text' => 'Instead use L</toLOWER_utf8>.
...
Memory obtained by this should B<ONLY> be freed with L<"Safefree">.
	...
	void	Newxc(void* ptr, int nitems, type, cast)','name' => 'Newxc'},'lex_stuff_pvs' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
C<SvSetMagicSV_nosteal>.
	...
	void	sv_setsv(SV *dstr, SV *sstr)','name' => 'sv_setsv'},'PerlIO_stdin' => {'text' => '','name' => 'PerlIO_stdin'},'toTITLE' => {'text' => 'Converts the specified character to titlecase.  If the input is anything but an
...
concept used in locale handling, so there is no functionality for that.)
	...
	U8	toTITLE(U8 ch)','name' => 'toTITLE'},'READ_XDIGIT' => {'text' => 'Returns the value of an ASCII-range hex digit and advances the string pointer.
...
effects.  Since we don\'t have to store a temporary value, it\'s faster.
	...
	SV*	SvREFCNT_inc_simple(SV* sv)','name' => 'SvREFCNT_inc_simple'},'XopENTRYCUSTOM' => {'text' => 'Exactly like C<XopENTRY(XopENTRY(Perl_custom_op_xop(aTHX_ o), which)> but more
efficient.  The I<which> parameter is identical to L</XopENTRY>.

		XopENTRYCUSTOM(const OP *o, which)','name' => 'XopENTRYCUSTOM'},'my_lstat' => {'text' => '','name' => 'my_lstat'},'my_atof' => {'text' => '','name' => 'my_atof'},'savestack_grow_cnt' => {'text' => '','name' => 'savestack_grow_cnt'},'PL_comppad_name' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.


During compilation, this points to the array containing the names part
of the pad for the currently-compiling code.','name' => 'PL_comppad_name'},'SvTRUE_nomg' => {'text' => 'Returns a boolean indicating whether Perl would evaluate the SV as true or
...
See L<perlguts/"Exception Handling">.
	...
	bool	sv_tainted(SV *const sv)','name' => 'sv_tainted'},'mro_set_private_data' => {'text' => '','name' => 'mro_set_private_data'},'do_gv_dump' => {'text' => '','name' => 'do_gv_dump'},'reg_named_buff_nextkey' => {'text' => '','name' => 'reg_named_buff_nextkey'},'hv_fill' => {'text' => 'Returns the number of hash buckets that
...
the hash is split.
	...
	STRLEN	hv_fill(HV *const hv)','name' => 'hv_fill'},'cv_const_sv' => {'text' => 'If C<cv> is a constant sub eligible for inlining, returns the constant
...
The reference count for the SV is set to 1.
	...
	SV*	newSVnv(const NV n)','name' => 'newSVnv'},'HeVAL' => {'text' => 'Returns the value slot (type C<SV*>)
...
C<isIDCONT_LC_utf8>.
	...
	bool	isIDCONT(char ch)','name' => 'isIDCONT'},'toLOWER_L1' => {'text' => 'Converts the specified Latin1 character to lowercase.  The results are undefined if
the input doesn\'t fit in a byte.

	U8	toLOWER_L1(U8 ch)','name' => 'toLOWER_L1'},'SvGETMAGIC' => {'text' => 'Invokes C<mg_get> on an SV if it has \'get\' magic.  For example, this
...
Only use when you are sure SvNOK is true.  See also C<SvNV()>.
	...
	U32	SvIOK(SV* sv)','name' => 'SvIOK'},'newANONHASH' => {'text' => '','name' => 'newANONHASH'},'PadnameSV' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
typedef for C<char>.
	...
	U32	HeUTF8(HE* he)','name' => 'HeUTF8'},'newRANGE' => {'text' => 'Constructs and returns a C<range> op, with subordinate C<flip> and
...
On EBCDIC platforms, you almost always want to use the C<isCNTRL_L1> variant.
	...
	bool	isCNTRL(char ch)','name' => 'isCNTRL'},'PadlistNAMESARRAY' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.


The C array of pad names.

	PADNAME ** PadlistNAMESARRAY(PADLIST padlist)','name' => 'PadlistNAMESARRAY'},'cophh_delete_pvn' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
Usually used via one of its frontends C<sv_vsetpvf> and C<sv_vsetpvf_mg>.
	...
	void	sv_vsetpvfn(SV *const sv, const char *const pat,
		            ...
		            va_list *const args,
		            SV **const svargs, const I32 svmax,
		            bool *const maybe_tainted)','name' => 'sv_vsetpvfn'},'PerlIO_get_cnt' => {'text' => '','name' => 'PerlIO_get_cnt'},'PerlIO_setlinebuf' => {'text' => '','name' => 'PerlIO_setlinebuf'},'utf16_to_utf8_reversed' => {'text' => '','name' => 'utf16_to_utf8_reversed'},'cast_ulong' => {'text' => '','name' => 'cast_ulong'},'to_utf8_case' => {'text' => 'C<p> contains the pointer to the UTF-8 string encoding
...
%utf8::ToLower.
	...
	char*	POPpbytex','name' => 'POPpbytex'},'cophh_store_sv' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
convenient to wrap the L</PL_rpeepp> hook.
	...
	void	SvSetMagicSV(SV* dsv, SV* ssv)','name' => 'SvSetMagicSV'},'isALPHANUMERIC' => {'text' => 'Returns a boolean indicating whether the specified character is a either an
...
in terms of this function.
	...
	void	sv_catpvn_flags(SV *const dstr,
		                const char *sstr,
		                const STRLEN len,
		                const I32 flags)','name' => 'sv_catpvn_flags'},'OP_TYPE_IS' => {'text' => 'Returns true if the given OP is not a NULL pointer
...
the NULL pointer check.
	...
	bool	OP_TYPE_IS(OP *o, Optype type)','name' => 'OP_TYPE_IS'},'PerlIO_flush' => {'text' => '','name' => 'PerlIO_flush'},'sv_2iv_flags' => {'text' => 'Return the integer value of an SV, doing any necessary string
conversion.  If flags includes SV_GMAGIC, does an mg_get() first.
...
string/length pair, and no precomputed hash.
	...
	SV *	cop_hints_fetch_pvs(const COP *cop,
		                    const char *key, U32 flags)','name' => 'cop_hints_fetch_pvs'},'cop_store_label' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
element.  Does not use C<TARG>.  See also C<PUSHs>, C<XPUSHmortal> and C<XPUSHs>.
	...
	PerlInterpreter* perl_alloc()','name' => 'perl_alloc'},'sv_dump' => {'text' => 'Dumps the contents of an SV to the C<STDERR> filehandle.

For an example of its output, see L<Devel::Peek>.

	void	sv_dump(SV* sv)','name' => 'sv_dump'},'pad_tidy' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
second, C<s2>.  Returns true or false.
	...
	bool	strLE(char* s1, char* s2)','name' => 'strLE'},'newSVpvn_flags' => {'text' => 'Creates a new SV and copies a string (which may contain C<NUL> (C<\\0>)
characters) into it.  The reference count for the
...
will have a reference count of 1, and the RV will be returned.
	...
	SV*	sv_setref_nv(SV *const rv,
		             const char *const classname,
		             const NV nv)','name' => 'sv_setref_nv'},'lex_stuff_pvn' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
function is more convenient.
	...
	void	lex_stuff_pvn(const char *pv, STRLEN len,
		              U32 flags)','name' => 'lex_stuff_pvn'},'eval_pv' => {'text' => 'Tells Perl to C<eval> the given string and return an SV* result.
...
is null, the other is returned unchanged.
	...
	OP *	op_append_elem(I32 optype, OP *first, OP *last)','name' => 'op_append_elem'},'lex_read_unichar' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
...
is returned.
	...
	void	PoisonFree(void* dest, int nitems, type)','name' => 'PoisonFree'},'gv_SVadd' => {'text' => '','name' => 'gv_SVadd'},'hv_common' => {'text' => '','name' => 'hv_common'},'XopENTRY' => {'text' => 'Return a member of the XOP structure.  I<which> is a cpp token
...
C<XOP *> from a C<OP *>, use the more efficient L</XopENTRYCUSTOM> instead.
		...
		XopENTRY(XOP *xop, which)','name' => 'XopENTRY'},'newSVREF' => {'text' => '','name' => 'newSVREF'},'toUPPER' => {'text' => 'Converts the specified character to uppercase.  If the input is anything but an
ASCII lowercase character, that input character itself is returned.  Variant
C<toUPPER_A> is equivalent.

	U8	toUPPER(U8 ch)','name' => 'toUPPER'},'sv_catpvs_flags' => {'text' => 'Like C<sv_catpvn_flags>, but takes a literal C<NUL>-terminated string instead
of a string/length pair.
...
L<http://www.unicode.org/unicode/reports/tr21/> (Case Mappings).
	...
	I32	foldEQ_utf8(const char *s1, char **pe1, UV l1,
		            bool u1, const char *s2, char **pe2,
		            UV l2, bool u2)','name' => 'foldEQ_utf8'},'Nullch' => {'text' => 'Null character pointer.  (No longer available when C<PERL_CORE> is
defined.)','name' => 'Nullch'},'sv_copypv_nomg' => {'text' => 'Like sv_copypv, but doesn\'t invoke get magic first.
...
the L</croak_sv> function, which does not involve clobbering C<ERRSV>.
	...
	void	vcroak(const char *pat, va_list *args)','name' => 'vcroak'},'parse_fullexpr' => {'text' => 'NOTE: this function is experimental and may change or be
removed without notice.
