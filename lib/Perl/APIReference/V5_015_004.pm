package Perl::APIReference::V5_015_004;
use strict;
use warnings;
...
set as required.
	...
	void	SvSetMagicSV_nosteal(SV* dsv, SV* ssv)','name' => 'SvSetMagicSV_nosteal'},'hv_undef' => {'text' => 'Undefines the hash.  The XS equivalent of undef(%hash).
...
compile time.)
	...
	CV*	newCONSTSUB_flags(HV* stash, const char* name,
		                  U32 flags, SV* sv)','name' => 'newCONSTSUB_flags'},'swash_init' => {'text' => '','name' => 'swash_init'},'newANONSUB' => {'text' => '','name' => 'newANONSUB'},'warn' => {'text' => 'This is an XS interface to Perl\'s C<warn> function.
...
C<mXPUSHi>.
	...
	void	SvLEN_set(SV* sv, STRLEN len)','name' => 'SvLEN_set'},'new_numeric' => {'text' => '','name' => 'new_numeric'},'to_utf8_lower' => {'text' => 'Convert the UTF-8 encoded character at p to its lowercase version and
store that in UTF-8 in ustrp and its length in bytes in lenp.  Note
that the ustrp needs to be at least UTF8_MAXBYTES_CASE+1 bytes since the
...
Individual members of C<PL_parser> have their own documentation.','name' => 'PL_parser'},'pv_uni_display' => {'text' => 'Build to the scalar dsv a displayable version of the string spv,
...
string/length pair.
	...
	void	sv_catpvs_flags(SV* sv, const char* s,
		                I32 flags)','name' => 'sv_catpvs_flags'},'is_lvalue_sub' => {'text' => '','name' => 'is_lvalue_sub'},'is_utf8_string_loc' => {'text' => 'Like is_utf8_string() but stores the location of the failure (in the
case of "utf8ness failure") or the location s+len (in the case of
