package Perl::APIReference::V5_008_000;
use strict;
use warnings;
...
C<SvPVx> for a version which guarantees to evaluate sv only once.
	...
	char*	SvPV(SV* sv, STRLEN len)','name' => 'SvPV'},'is_utf8_string' => {'text' => 'Returns true if first C<len> bytes of the given string form a valid UTF8
...
usually preferable for finding the value of a key.
	...
	void*	HeKEY(HE* he)','name' => 'HeKEY'},'pack_cat' => {'text' => 'The engine implementing pack() Perl function.
...
copy-ish functions and macros use this underneath.
	...
	void	sv_setsv_flags(SV* dsv, SV* ssv, I32 flags)','name' => 'sv_setsv_flags'},'sv_utf8_upgrade_flags' => {'text' => 'Convert the PV of an SV to its UTF8-encoded form.
...
Allows length and flags to be passed to low level routine.
	...
	UV	utf8n_to_uvchr(U8 *s, STRLEN curlen, STRLEN* retlen, U32 flags)','name' => 'utf8n_to_uvchr'},'sv_utf8_upgrade' => {'text' => 'Convert the PV of an SV to its UTF8-encoded form.
Forces the SV to string form if it is not already.
Always sets the SvUTF8 flag to avoid future validity checks even
...
C<xsubpp>.  See L<perlxs/"The VERSIONCHECK: Keyword">.
	...
	bool	SvROK(SV* sv)','name' => 'SvROK'},'new_vstring' => {'text' => 'Returns a pointer to the next character after the parsed
...
passed in.
	...
	char*	new_vstring(char *vstr, SV *sv)','name' => 'new_vstring'},'sv_unref' => {'text' => 'Unsets the RV status of the SV, and decrements the reference count of
...
false.
	...
	bool	strNE(char* s1, char* s2)','name' => 'strNE'},'SvUTF8_on' => {'text' => 'Turn on the UTF8 status of an SV (the data is not changed, just the flag).
...
be freed with the C<Safefree()> function.
	...
	char	toLOWER(char ch)','name' => 'toLOWER'},'bytes_from_utf8' => {'text' => 'Converts a string C<s> of length C<len> from UTF8 into byte encoding.
...
would lose the UTF-8\'ness of the PV.
	...
	char*	SvPVutf8_nolen(SV* sv)','name' => 'SvPVutf8_nolen'},'sv_setnv_mg' => {'text' => 'Like C<sv_setnv>, but also handles \'set\' magic.
...
if necessary.  See also C<sv_cmp_locale>.  See also C<sv_cmp>.
	...
	I32	sv_cmp_locale(SV* sv1, SV* sv2)','name' => 'sv_cmp_locale'},'uvuni_to_utf8_flags' => {'text' => 'Adds the UTF8 representation of the Unicode codepoint C<uv> to the end
...
up past C<e>, croaks.
	...
	char*	SvPVutf8_force(SV* sv, STRLEN len)','name' => 'SvPVutf8_force'},'sv_2pvutf8_nolen' => {'text' => 'Return a pointer to the UTF8-encoded representation of the SV.
May cause the SV to be upgraded to UTF8 as a side-effect.

Usually accessed via the C<SvPVutf8_nolen> macro.

	char*	sv_2pvutf8_nolen(SV* sv)','name' => 'sv_2pvutf8_nolen'},'SvTAINTED_on' => {'text' => 'Marks an SV as tainted.
...
some level of strict-ness.
	...
	void	sv_nolocking(SV *)','name' => 'sv_nolocking'},'SVt_NV' => {'text' => 'Double type flag for scalars.  See C<svtype>.','name' => 'SVt_NV'},'sv_vcatpvfn' => {'text' => 'Processes its arguments like C<vsprintf> and appends the formatted output
...
of the result may be zero.
	...
	GV*	gv_fetchmeth_autoload(HV* stash, const char* name, STRLEN len, I32 level)','name' => 'gv_fetchmeth_autoload'},'sv_nounlocking' => {'text' => 'Dummy routine which "unlocks" an SV when there is no locking module present.
Exists to avoid test for a NULL function pointer and because it could potentially warn under
some level of strict-ness.

	void	sv_nounlocking(SV *)','name' => 'sv_nounlocking'},'sv_2pvutf8' => {'text' => 'Return a pointer to the UTF8-encoded representation of the SV, and set *lp
to its length.  May cause the SV to be upgraded to UTF8 as a side-effect.

Usually accessed via the C<SvPVutf8> macro.

	char*	sv_2pvutf8(SV* sv, STRLEN* lp)','name' => 'sv_2pvutf8'},'dITEMS' => {'text' => 'Sets up the C<items> variable.
...
empty.
	...
	I32	av_len(AV* ar)','name' => 'av_len'},'sv_unmagic' => {'text' => 'Removes all magic of type C<type> from an SV.
...
sv only once. Use the more efficient C<SvNV> otherwise.
	...
	NV	SvNVx(SV* sv)','name' => 'SvNVx'},'unpack_str' => {'text' => 'The engine implementing unpack() Perl function.
...
B<private> setting.  Use C<SvNOK>.
	...
	bool	SvNOKp(SV* sv)','name' => 'SvNOKp'},'StructCopy' => {'text' => 'This is an architecture-independent macro to copy one structure to another.
...
items on the stack.  See L<perlxs/"Variable-length Parameter Lists">.
	...
	I32	items','name' => 'items'},'bytes_to_utf8' => {'text' => 'Converts a string C<s> of length C<len> from ASCII into UTF8 encoding.
...
cope with complex macro expressions. Always use the macro instead.
	...
	char*	sv_pvn(SV *sv, STRLEN *len)','name' => 'sv_pvn'},'XST_mPV' => {'text' => 'Place a copy of a string into the specified position C<pos> on the stack. 
...
(but note, as explained above, that there may be more.)
	...
	UV	to_utf8_title(U8 *p, U8* ustrp, STRLEN *lenp)','name' => 'to_utf8_title'},'PUSHn' => {'text' => 'Push a double onto the stack.  The stack must have room for this element.
...
The reference count for the SV is set to 1.
	...
	SV*	newSVuv(UV u)','name' => 'newSVuv'},'SvNVX' => {'text' => 'Returns the raw value in the SV\'s NV slot, without checks or conversions.
Only use when you are sure SvNOK is true. See also C<SvNV()>.

	NV	SvNVX(SV* sv)','name' => 'SvNVX'},'sv_nosharing' => {'text' => 'Dummy routine which "shares" an SV when there is no sharing module present.
Exists to avoid test for a NULL function pointer and because it could potentially warn under
some level of strict-ness.

	void	sv_nosharing(SV *)','name' => 'sv_nosharing'},'gv_fetchmeth' => {'text' => 'Returns the glob with the given C<name> and a defined subroutine or
...
and leaves the UTF8 status as it was.
	...
	int	mg_get(SV* sv)','name' => 'mg_get'},'sv_pos_u2b' => {'text' => 'Converts the value pointed to by offsetp from a count of UTF8 chars from
...
Usually accessed via the C<SvPVbyte_nolen> macro.
	...
	char*	sv_2pvbyte_nolen(SV* sv)','name' => 'sv_2pvbyte_nolen'},'perl_free' => {'text' => 'Releases a Perl interpreter.  See L<perlembed>.
...
will be returned and will have a reference count of 1.
	...
	SV*	sv_setref_nv(SV* rv, const char* classname, NV nv)','name' => 'sv_setref_nv'},'uvchr_to_utf8' => {'text' => 'Adds the UTF8 representation of the Native codepoint C<uv> to the end
of the string C<d>; C<d> should be have at least C<UTF8_MAXLEN+1> free
bytes available. The return value is the pointer to the byte after the
end of the new character. In other words,
...
-- the Boyer-Moore algorithm.
	...
	void	fbm_compile(SV* sv, U32 flags)','name' => 'fbm_compile'},'sv_utf8_downgrade' => {'text' => 'Attempt to convert the PV of an SV from UTF8-encoded to byte encoding.
...
C<SPAGAIN>.','name' => 'SP'},'SvSetSV_nosteal' => {'text' => 'Calls a non-destructive version of C<sv_setsv> if dsv is not the same as
...
Will also turn off the UTF8 status.
	...
	void	SvPOK_only(SV* sv)','name' => 'SvPOK_only'},'sv_catpvn_mg' => {'text' => 'Like C<sv_catpvn>, but also handles \'set\' magic.
...
cope with complex macro expressions. Always use the macro instead.
	...
	SV*	SvRV(SV* sv)','name' => 'SvRV'},'Nullch' => {'text' => 'Null character pointer.
=for hackers
Found in file handy.h','name' => 'Nullch'},'mg_copy' => {'text' => 'Copies the magic from one SV to another.  See C<sv_magic>.
...
to accommodate the addition.
	...
	void	av_push(AV* ar, SV* val)','name' => 'av_push'},'POPn' => {'text' => 'Pops a double off the stack.
...
SV is B<not> incremented.
	...
	SV*	newRV_noinc(SV *sv)','name' => 'newRV_noinc'},'SvUVx' => {'text' => 'Coerces the given SV to an unsigned integer and returns it. Guarantees to
