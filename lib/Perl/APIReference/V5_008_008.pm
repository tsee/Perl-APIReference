...
do{$VAR1 = {'SvPV_set' => {'text' => 'Set the value of the PV pointer in sv to val.  See C<SvIV_set>.
...
C<XPUSHu> and C<mXPUSHu>.
	...
	void	PUSHu(UV uv)','name' => 'PUSHu'},'sv_vsetpvf_mg' => {'text' => 'Like C<sv_vsetpvf>, but also handles \'set\' magic.
...
http://www.unicode.org/unicode/reports/tr21/ (Case Mappings).
	...
	I32	ibcmp_utf8(const char* a, char **pe1, UV l1, bool u1, const char* b, char **pe2, UV l2, bool u2)','name' => 'ibcmp_utf8'},'mXPUSHp' => {'text' => 'Push a string onto the stack, extending the stack if necessary.  The C<len>
...
use the Encode extension for that.
	...
	bool	SvIOK_notUV(SV* sv)','name' => 'SvIOK_notUV'},'G_ARRAY' => {'text' => 'Used to indicate list context.  See C<GIMME_V>, C<GIMME> and
...
C<mXPUSHu> and C<XPUSHu>.
	...
	void	mPUSHu(UV uv)','name' => 'mPUSHu'},'gv_stashpvn' => {'text' => 'Returns a pointer to the stash for a specified package.  C<name> should
...
package does not exist then NULL is returned.
	...
	HV*	gv_stashpvn(const char* name, U32 namelen, I32 create)','name' => 'gv_stashpvn'},'call_argv' => {'text' => 'Performs a callback to the specified Perl sub.  See L<perlcall>.
...
macros.
	...
	NV	sv_2nv(SV* sv)','name' => 'sv_2nv'},'sv_setsv_nomg' => {'text' => 'Like C<sv_setsv> but doesn\'t process magic.

	void	sv_setsv_nomg(SV* dsv, SV* ssv)','name' => 'sv_setsv_nomg'},'SvSetSV' => {'text' => 'Calls C<sv_setsv> if dsv is not the same as ssv.  May evaluate arguments
more than once.
...
be freed with the C<Safefree()> function.
	...
	char	toLOWER(char ch)','name' => 'toLOWER'},'SvNV_set' => {'text' => 'Set the value of the NV pointer in sv to val.  See C<SvIV_set>.
...
C<hv_iterkey>.
	...
	char*	sv_reftype(SV* sv, int ob)','name' => 'sv_reftype'},'SvUV_set' => {'text' => 'Set the value of the UV pointer in sv to val.  See C<SvIV_set>.
...
COW)
	...
	bool	SvIsCOW(SV* sv)','name' => 'SvIsCOW'},'CopyD' => {'text' => 'Like C<Copy> but returns dest. Useful for encouraging compilers to tail-call
optimise.

	void *	CopyD(void* src, void* dest, int nitems, type)','name' => 'CopyD'},'sv_unmagic' => {'text' => 'Removes all magic of type C<type> from an SV.
...
must be called prior to setup the C<MARK> variable.
	...
	int	mg_clear(SV* sv)','name' => 'mg_clear'},'dAXMARK' => {'text' => 'Sets up the C<ax> variable and stack marker variable C<mark>.
This is usually handled automatically by C<xsubpp> by calling C<dXSARGS>.
...
and C<XPUSHn>.
	...
	void	mPUSHn(NV nv)','name' => 'mPUSHn'},'gv_stashpv' => {'text' => 'Returns a pointer to the stash for a specified package.  C<name> should
...
L<perlcall>.','name' => 'G_SCALAR'},'sv_vcatpvf_mg' => {'text' => 'Like C<sv_vcatpvf>, but also handles \'set\' magic.

Usually used via its frontend C<sv_catpvf_mg>.

	void	sv_vcatpvf_mg(SV* sv, const char* pat, va_list* args)','name' => 'sv_vcatpvf_mg'},'SvPVutf8x_force' => {'text' => 'Like C<SvPV_force>, but converts sv to utf8 first if necessary.
...
instead.
	...
	SV*	sv_newref(SV* sv)','name' => 'sv_newref'},'Newxc' => {'text' => 'The XSUB-writer\'s interface to the C C<malloc> function, with
cast.

	void	Newxc(void* ptr, int nitems, type, cast)','name' => 'Newxc'},'hv_store' => {'text' => 'Stores an SV in a hash.  The hash key is specified as C<key> and C<klen> is
...
Usually used via its frontend C<sv_setpvf>.
	...
	void	sv_vsetpvf(SV* sv, const char* pat, va_list* args)','name' => 'sv_vsetpvf'},'isUPPER' => {'text' => 'Returns a boolean indicating whether the C C<char> is an uppercase
character.
...
B<private> setting.  Use C<SvNOK>.
	...
	bool	SvNOKp(SV* sv)','name' => 'SvNOKp'},'Newx' => {'text' => 'The XSUB-writer\'s interface to the C C<malloc> function.

	void	Newx(void* ptr, int nitems, type)','name' => 'Newx'},'XSRETURN_UV' => {'text' => 'Return an integer from an XSUB immediately.  Uses C<XST_mUV>.
...
wrapper instead.
	...
	bool	sv_tainted(SV* sv)','name' => 'sv_tainted'},'sv_catsv_nomg' => {'text' => 'Like C<sv_catsv> but doesn\'t process magic.

	void	sv_catsv_nomg(SV* dsv, SV* ssv)','name' => 'sv_catsv_nomg'},'sv_grow' => {'text' => 'Expands the character buffer in the SV.  If necessary, uses C<sv_unref> and
...
memory tracking tools, valgrind for example.
	...
	void	Newxz(void* ptr, int nitems, type)','name' => 'Newxz'},'SVt_PVHV' => {'text' => 'Type flag for hashes.  See C<svtype>.','name' => 'SVt_PVHV'},'pv_uni_display' => {'text' => 'Build to the scalar dsv a displayable version of the string spv,
...
coercion.  See also C<SvCUR>, which gives raw access to the xpv_cur slot.
	...
	STRLEN	sv_len(SV* sv)','name' => 'sv_len'},'warn' => {'text' => 'This is the XSUB-writer\'s interface to Perl\'s C<warn> function.  Call this
...
C<SvIV_set> instead of the lvalue assignment to C<SvIVX>.
	...
	void	SvIV_set(SV* sv, IV val)','name' => 'SvIV_set'},'SvIV' => {'text' => 'Coerces the given SV to an integer and returns it. See  C<SvIVx> for a
...
and leaves the UTF-8 status as it was.
	...
	int	mg_get(SV* sv)','name' => 'mg_get'},'sv_vcatpvf' => {'text' => 'Processes its arguments like C<vsprintf> and appends the formatted output
...
statement boundaries.  See also C<sv_newmortal> and C<sv_2mortal>.
	...
	char*	POPpbytex','name' => 'POPpbytex'},'sv_catpvn_nomg' => {'text' => 'Like C<sv_catpvn> but doesn\'t process magic.
...
removed without notice.
	...
	bool	sv_utf8_decode(SV *sv)','name' => 'sv_utf8_decode'},'savepvn' => {'text' => 'Perl\'s version of what C<strndup()> would be if it existed. Returns a
...
valid UTF-8; if the original SV was bytes, the pattern should be too.
	...
	bool	SvIOK_UV(SV* sv)','name' => 'SvIOK_UV'},'XSRETURN_PV' => {'text' => 'Return a copy of a string from an XSUB immediately.  Uses C<XST_mPV>.
...
optimise.
	...
	void *	MoveD(void* src, void* dest, int nitems, type)','name' => 'MoveD'},'strLE' => {'text' => 'Test two strings to see if the first, C<s1>, is less than or equal to the
...
C<PUSHmortal> and C<PUSHs>.
	...
	void	XPUSHmortal()','name' => 'XPUSHmortal'},'eval_pv' => {'text' => 'Tells Perl to C<eval> the given string and return an SV* result.
...
NOTE: the perl_ form of this function is deprecated.
	...
	void	SvRV_set(SV* sv, SV* val)','name' => 'SvRV_set'},'SvPVbyte' => {'text' => 'Like C<SvPV>, but converts sv to byte representation first if necessary.
...
appending to the currently-stored string.
	...
	int	AvFILL(AV* av)','name' => 'AvFILL'},'SvSTASH_set' => {'text' => 'Set the value of the STASH pointer in sv to val.  See C<SvIV_set>.

	void	SvSTASH_set(SV* sv, STASH* val)','name' => 'SvSTASH_set'},'SvPVutf8x' => {'text' => 'Like C<SvPV>, but converts sv to utf8 first if necessary.
...
possible to set C<*st> and C<*gvp> to the stash and GV associated with it.
	...
	CV*	sv_2cv(SV* sv, HV** st, GV** gvp, I32 lref)','name' => 'sv_2cv'},'ZeroD' => {'text' => 'Like C<Zero> but returns dest. Useful for encouraging compilers to tail-call
optimise.

	void *	ZeroD(void* dest, int nitems, type)','name' => 'ZeroD'},'SvPV_force_nomg' => {'text' => 'Like C<SvPV> but will force the SV into containing just a string
(C<SvPOK_only>).  You want force if you are going to update the C<SvPVX>
...
cope with complex macro expressions. Always use the macro instead.
	...
	SV*	hv_scalar(HV* hv)','name' => 'hv_scalar'},'Nullch' => {'text' => 'Null character pointer.','name' => 'Nullch'},'mg_copy' => {'text' => 'Copies the magic from one SV to another.  See C<sv_magic>.
...
number may use \'_\' characters to separate digits.
	...
	UV	grok_oct(char* start, STRLEN* len_p, I32* flags, NV *result)','name' => 'grok_oct'},'MARK' => {'text' => 'Stack marker variable for the XSUB.  See C<dMARK>.','name' => 'MARK'},'scan_hex' => {'text' => 'For backwards compatibility. Use C<grok_hex> instead.
